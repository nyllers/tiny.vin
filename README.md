...
`destinations.title` holds the target webpage's `<title>`, fetched once (`fetchPageTitle` in `src/index.js`) when a URL destination is first created and shown above the URL on its card. The response is streamed and reading stops as soon as `<title>` is matched (usually within the first few KB), capped at 1MB total to bound worst-case latency for pages that front-load `<head>` with large inline scripts before `<title>` (e.g. YouTube) — and any failure (timeout, no `<title>`, non-HTML response) just leaves it `NULL` rather than blocking the redirect from being created. E-mail destinations have no webpage to fetch a title from, so this column stays `NULL` for those, and it's never refetched for a destination that already has one (including `NULL` from a failed fetch — there's no retry). If the database predates this, apply `migrations/0019_add_destination_title.sql` (`wrangler d1 execute tiny-vin-db --file=migrations/0019_add_destination_title.sql --remote`) in addition to `schema.sql`.
