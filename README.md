...
The `/stats` page (`GET /api/stats`) surfaces this data per-account: total links/redirects, a breakdown of redirects per URL, and — grouped across all of that account's URLs — a daily count for the last 14 days, top countries, top referrers (by hostname, with no referer shown as "Direct"), and a rough browser/device split parsed from the user agent string.
