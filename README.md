...
The `/emails` page (linked from the nav) lets a signed-in account redirect `<alias>@tiny.vin` to any other e-mail address — e-mail redirects are `aliases` rows with `kind = 'email'` (see "Destinations and aliases" above), with `alias@tiny.vin` reserved words checked against the same `RESERVED_CODES` set used for paths.
