...
In addition to path-based short URLs (`tiny.vin/<code>`), a URL can also get a subdomain-based one (`<name>.tiny.vin`) via the "Add subdomain" button on each card. `aliases.kind` (`'generated-path'`, `'custom-path'`, `'subdomain'`, or `'email'` — the first two distinguish an auto-generated code from a user-chosen pathname) distinguishes them, and `(code, kind)` is a unique constraint (not the primary key — see "Destinations and aliases" below), so the same string can be used as a path and a subdomain independently.
...
Every redirect a signed-in account creates — a URL path/subdomain/generated code, or an e-mail alias — is a row in `aliases`, keyed by a real auto-incrementing `id` rather than `(code, kind)` (which stays as a `UNIQUE` constraint, preserving the collision rules described above and in "E-mail redirects" below). What an alias points to lives separately, in `destinations`: one row per `(owner, target)` pair, where the target is either a website URL or an e-mail address depending on the alias's `kind`. `aliases.destination_id` references it.

This split exists because multiple aliases routinely point at the same target — "Add Path"/"Add Subdomain"/"Generate Path" on a URL card, or "Add alias" on an e-mail card, all attach another alias to the *same* destination — and previously each of those rows duplicated the full target text. Creating an alias now finds-or-creates its destination (`findOrCreateDestination` in `src/index.js`) instead of inserting the text again; deleting an alias deletes its destination too if it was the last one pointing there. `aliases.cloudflare_rule_id` is `NULL` for every `kind` except `'email'` — see "E-mail redirects" below for what it's for.

Since `aliases` now holds both URL and e-mail rows for the same account, every query that used to scope by `created_by` alone now also filters on `kind` (`!= 'email'` for URL-facing pages, `= 'email'` for e-mail-facing ones) to keep the two from leaking into each other's lists.

If the database predates this, apply `migrations/0017_split_destinations_and_aliases.sql` (`wrangler d1 execute tiny-vin-db --file=migrations/0017_split_destinations_and_aliases.sql --remote`) in addition to `schema.sql`. This migration replaces `urls` and `email_redirects` (dropping both) and repoints `redirect_events`/`email_redirect_events` at `aliases` by `alias_id` instead of `(code, kind)`/`alias` — back up first if the data matters.

`destinations.title` holds the target webpage's `<title>`, fetched once (`fetchPageTitle` in `src/index.js`) when a URL destination is first created and shown above the URL on its card. The response is streamed and reading stops as soon as `<title>` is matched (usually within the first few KB), capped at 1MB total to bound worst-case latency for pages that front-load `<head>` with large inline scripts before `<title>` (e.g. YouTube). If no `<title>` can be found at all (timeout, non-HTML response, no tag present), `deriveFallbackTitle` falls back to the last non-empty path segment of the URL (ignoring the query string) instead of leaving the card with no title. E-mail destinations have no webpage to fetch a title from, so the destination's own e-mail address is used as its title instead — set once, directly, no fetch involved. Either way, the title is never refetched automatically once a destination has one (including a fallback — there's no retry), though it can always be edited by hand afterward — see "Editing a title" below. If the database predates this, apply `migrations/0019_add_destination_title.sql` (`wrangler d1 execute tiny-vin-db --file=migrations/0019_add_destination_title.sql --remote`) in addition to `schema.sql`. `scripts/backfill-titles.js` is the one-time (and re-runnable) backfill for destinations that predate the title feature entirely — see the comment at the top of the file for usage.

### Editing a title

Since a title is only ever fetched (or derived) once and never automatically refreshed, a URL card's title can be edited by hand at any time via the pencil icon next to it — this is the intended way to fix a wrong, stale, or unhelpful auto-fetched/fallback title, rather than re-fetching. `PATCH /api/redirects/title` (`{ destination, title }`) in `src/index.js` handles it, scoped to the signed-in account's own destinations; an empty title clears it back to none. It's deliberately restricted to URL destinations (`a.kind != 'email'`) — an e-mail destination's title is always its own address, and e-mail cards render only that title with no separate address line beneath it, so editing one away would make the real address disappear from the card.

## E-mail redirects

The home page (linked from the nav as "Redirects") lets a signed-in account redirect `<alias>@tiny.vin` to any other e-mail address, from the same form and card list used for URLs — pasting a URL or an e-mail address into the one input is enough to tell the two apart (checked client-side against an e-mail-address pattern; anything else is treated as a URL), and each card in the list carries a large, faint watermark icon showing which kind it is. E-mail redirects are `aliases` rows with `kind = 'email'` (see "Destinations and aliases" above), with `alias@tiny.vin` reserved words checked against the same `RESERVED_CODES` set used for paths. `/emails` and `/emails.html` (the page's old, separate URL from before this merge) now just 301-redirect to `/`.

Typing just a destination address into the same form used for URLs generates a random 8-character lowercase alphanumeric alias (`generateEmailAlias` in `src/index.js`), retrying on collision the same way `generateCode` does for URLs. Multiple aliases can point at the same destination — cards group by destination the same way URL cards group by original URL, and each card's "Add alias" button reveals an inline input (mirroring "Add Path"/"Add Subdomain" on a URL card) for attaching another chosen alias to that destination without re-entering it.
...
Cloudflare's catch-all rule only supports **forward** or **drop** actions — it can't target a Worker, so there's no single rule that routes every address through `aliases`. Instead, `handleCreateEmailRedirect` creates one **Email Routing Rule** per alias via the API (a literal match on `<alias>@tiny.vin` → action **Send to Worker** → `tiny-vin`), and `handleDeleteEmailRedirect` deletes it again when the redirect is removed. The rule's id is stored in `aliases.cloudflare_rule_id`. The Worker's `email()` export (in `src/index.js`) then just looks up the recipient's local part among `kind = 'email'` aliases and calls `message.forward()`.
...
Every message that matches a known alias is logged to `email_redirect_events`, mirroring `redirect_events` for URLs: sender address, subject, `Message-ID`, size in bytes, the full MIME headers as a JSON blob, whether `message.forward()` succeeded, and a reject reason if it didn't — useful for confirming a message actually reached this Worker and was handed off to Cloudflare, when delivery issues turn out to be on the receiving mail provider's side instead. `alias_id` is a foreign key into `aliases(id)` with `ON DELETE CASCADE`, so deleting a redirect cleans up its event history too. The raw message body is deliberately not stored. Recording happens via `ctx.waitUntil()`, is best-effort, and only runs for messages that match an existing alias (unmatched addresses are rejected without being logged, same as a 404 short URL isn't logged).

## Redirect statistics

Every successful redirect (path or subdomain) is logged to `redirect_events`: which alias was hit, a timestamp, and as much request detail as a Worker can see — IP, country, latitude/longitude, user agent, referer, the full request headers, and Cloudflare's whole `request.cf` object (geo, TLS, bot-management score, ASN, etc.), each as a JSON blob. `alias_id` is a foreign key into `aliases(id)` with `ON DELETE CASCADE`, so it's a proper many-to-one relationship (many events per short URL) and deleting a short URL cleans up its stats too — D1 enforces foreign keys by default, so this isn't just documentation. Recording happens in the background via `ctx.waitUntil()` so it never adds latency to the redirect itself, and it's best-effort (a logging failure never blocks or breaks the redirect).
...
The `/email-stats` page mirrors `/stats`, built from `email_redirect_events` instead of `redirect_events`. The "Breakdown Last 14 Days" section shows messages received per day, messages per alias, and messages by sender domain (the part of `from_address` after the `@`, falling back to "Unknown" when there isn't one) — there's no browser/referer/geo equivalent, since SMTP doesn't give a Worker anything like `request.cf`. "Total Messages by Destination" below it groups cards by destination address exactly like the home page's redirect list itself does, each card listing every alias that forwards there with its lifetime message count and last-message time. The summary tiles add one thing `/stats` doesn't have: **Failed to Forward**, a lifetime count of messages where `message.forward()` threw — worth watching if a destination stops accepting Cloudflare's forwards (see the Gmail delivery notes above). `GET /api/email-stats` powers this the same way `GET /api/stats` powers the URL page, and the two pages' shared chart-drawing code (bar charts, the expand-to-modal view, summary tiles) lives in `public/charts.js` rather than being duplicated.

## Change history

Every table has a matching `<table>_history` table (`login_identities_history`, `aliases_history`, `destinations_history`, `login_events_history`, `redirect_events_history`, `api_keys_history`, `email_redirect_events_history`) holding the same columns plus a `history_created` timestamp. Triggers on each table copy a row's content into its history table right before an `UPDATE` or `DELETE`, so nothing has to opt in from the application code — this includes rows removed indirectly, e.g. a `redirect_events` row disappearing via `aliases`' `ON DELETE CASCADE` still lands in `redirect_events_history`. History tables drop the original's primary/unique/foreign key constraints, since the same row can be changed many times over its life and a history row must never be blocked by a parent that's since been deleted itself.

`aliases_history` predates the `aliases` table itself — it started life as `urls_history`, renamed in place by `migrations/0017_split_destinations_and_aliases.sql`. Its pre-rename rows keep their original `code`/`kind`/`original_url` columns exactly as recorded at the time (including a batch of rows folded in from the now-dropped `email_redirects_history`); only rows recorded after that migration carry `id`/`destination_id`/`cloudflare_rule_id` instead. Both shapes coexist in the same table on purpose — a history table is a true snapshot of the row as it looked when captured, not something that gets retroactively normalized.
