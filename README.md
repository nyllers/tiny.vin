...
The `/stats` page (`GET /api/stats`) surfaces this data per-account: total links/redirects, redirects in the last 14 days, a breakdown of redirects per URL, and — grouped across all of that account's URLs and rendered as small SVG bar charts — a daily count for the last 14 days, top origin countries, top referrers (by hostname, with no referer shown as "Direct" and `tiny.vin`/its subdomains excluded as not being real external referrers), and a rough browser/device split parsed from the user agent string.
